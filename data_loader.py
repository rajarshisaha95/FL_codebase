...
def get_dataset(args):
    ...
    if dataset_name == 'cifar10':
        ...
        trainset, testset = get_svhn_dataset()
    elif dataset_name == 'tiny_imagenet':
        trainset, testset = get_tiny_imagenet_dataset()
    elif dataset_name == 'glue':
        trainset, testset = get_glue_dataset(args)
    else:
        raise ValueError('cifar10, cifar100 ,svhn, tiny_imagenet, glue')
...
def dirichlet_dist(labels, args):
    ...
    extra_sample = math.ceil(len(left_over_inds) / np.sum(np.asarray(dist) < (len(labels) / args.num_client)))
    given = 0
